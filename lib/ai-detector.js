...
function vocabularyRichness(text) {
  ...
  if (words.length < 20) return { score: 0, detail: 'Text too short' };
...
function zipfDeviation(text) {
  const words = tokenize(text);
  if (words.length < 30) return { score: 0, detail: 'Text too short' };
  const freq = {};
...
function transitionDensity(text) {
  ...
  // AI text: density 0.30-0.70, Human: 0.05-0.25
  const aiLikelihood = Math.max(0, Math.min(100, density * 150));
  return { score: aiLikelihood, density: density.toFixed(3), count, detail: `${count} transitions in ${sentences.length} sentences (density: ${density.toFixed(2)} — ${density > 0.40 ? 'high — AI pattern' : density > 0.20 ? 'moderate' : 'normal'})` };
}
...
function detectAIWritten(text) {
  ...
  const weights = {
    ...
    repetitionPatterns: 0.10,
    characterEntropy: 0.15,
  };

  let totalScore = 0;
  for (const key in weights) {
    totalScore += (metrics[key].score || 0) * weights[key];
  }
  ...
  return { score: totalScore, verdict, level, metrics };
}
